...
@st.cache_data
def load_data():
    # Ensure master_timetable.xlsx exists in the same directory
    df = pd.read_excel("master_timetable.xlsx", header=[2, 3])
    df = df.dropna(how='all', axis=0).dropna(how='all', axis=1)
    return df

# 2. High-Fidelity Word Document Generator (UPDATED FOR ALIGNMENT & SPACING)
def generate_formal_docx(sender, receiver, target_class, s_day, s_per, r_day, r_per, reason):
    ...
    # --- Info Fields (Using Borderless Table for perfect indentation alignment) ---
    # Creates a 2x2 table to align labels and content without underscores
    ...
    p_name_label = row1[0].paragraphs[0]
    p_name_label.add_run("Name of Teacher:").bold = True [cite: 3]
    row1[1].text = f"\t{sender}" [cite: 3]

    # Reason Row
    row2 = info_table.rows[1].cells
    p_reason_label = row2[0].paragraphs[0]
    p_reason_label.add_run("Reason for Exchange:").bold = True [cite: 4]
    row2[1].text = f"\t{reason if reason else ''}" [cite: 4]

    # Add spacing before main table
    spacer = doc.add_paragraph()
    spacer.paragraph_format.space_before = Pt(12)

    # --- 14-column Main Table (2 headers + 4 content rows = 6 rows total) ---
    table = doc.add_table(rows=6, cols=14) [cite: 5]
    ...
    # Fill Row 3 (Index 2) with actual data 
    # Substituted Lesson
    ...
    # Rows 4, 5, and 6 (Indices 3, 4, 5) remain empty as requested 

    # --- Footer Section (Bottom Table with Alignment) ---
    footer_spacer = doc.add_paragraph()
    footer_spacer.paragraph_format.space_before = Pt(18)
    ...
    # Row 1: Signatures
    ft.cell(0, 0).text = f"Signature of teacher: ____________________" [cite: 6]
    
    cell_principal = ft.cell(0, 1)
    p_principal = cell_principal.paragraphs[0]
    p_principal.text = "Approved by Principal: ____________________" [cite: 6]
    p_principal.alignment = WD_ALIGN_PARAGRAPH.RIGHT # Right aligned

    # Row 2: Dates
    ft.cell(1, 0).text = f"Date: {today}" [cite: 6]
    
    cell_date = ft.cell(1, 1)
    p_date = cell_date.paragraphs[0]
    p_date.text = "Date: ____________________" [cite: 6]
    p_date.alignment = WD_ALIGN_PARAGRAPH.RIGHT # Right aligned
...
try:
    ...
    with tab1:
        ...
        # Wrapping selection in an expander that can be collapsed
        with st.expander("Selection Filters", expanded=True):
